...
class VariationalDropout(nn.Module):
    ...
    def forward(self, x):
        # Here we apply a Bernoulli distribution to sample the dropout mask
        dropout_mask = torch.bernoulli(torch.full_like(x, 1 - self.dropout_rate)).to(x.device)
        return x * dropout_mask / (1 - self.dropout_rate)  # Scaled dropout to maintain the expected value

# Define the VRNN model with variational dropout
class VRNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, z_dim, n_layers, n_classes, dropout_rate=0.2):
        ...
        self.phi_x = nn.Sequential(
            ...
            nn.ReLU(),
            VariationalDropout(hidden_dim, self.dropout_rate),  # Add variational dropout here
...
def bayesian_inference(model, dataloader, device, num_samples=10):
    model.eval()
    all_preds = []
    all_uncertainties = []
    ...
    with torch.no_grad():
        for inputs, _ in dataloader:
            ...
            # Aggregate the predictions (mean across samples for classification)
